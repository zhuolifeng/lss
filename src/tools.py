...
def get_val_info(model, valloader, loss_fn, device, use_tqdm=False):
    ...
    for batch in loader:
        ...
        # 攻击需要梯度，所以不使用no_grad
        corrputed_images = imagecorruption.apply_corruption(allimgs.to(device), rots.to(device), trans.to(device), intrins.to(device), post_rots.to(device), post_trans.to(device), binimgs.to(device), type='robust_bev', intensity='medium')
